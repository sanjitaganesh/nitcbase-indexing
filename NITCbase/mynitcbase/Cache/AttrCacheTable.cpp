...
{
    // validate relId is within the bounds of the open relation table [0, MAX_OPEN)
    if (relId < 0 || relId >= MAX_OPEN)
        return E_OUTOFBOUND;

    // if attrCache[relId] is null, the relation has not been opened yet
    if (attrCache[relId] == nullptr)
        return E_RELNOTOPEN;

    // traverse the linked list of AttrCacheEntry nodes for this relation
    // each node holds the cached AttrCatEntry for one attribute
    for (AttrCacheEntry *entry = attrCache[relId]; entry != nullptr; entry = entry->next)
    {
        // offset uniquely identifies an attribute within a relation
        if (entry->attrCatEntry.offset == attrOffset)
        {
            // overwrite the cached AttrCatEntry fields with values from attrCatBuf
            // (this is a field-by-field copy since AttrCatEntry contains char arrays,
            //  so direct struct assignment may not be safe for string fields)
            ...
            // mark dirty = true so this entry gets written back to disk
            // when the relation is closed (via closeRel)
            entry->dirty = true;
    ...
    // no attribute with the given offset exists in this relation's cache
    return E_ATTRNOTEXIST;
}
...
int AttrCacheTable::setAttrCatEntry(int relId, char attrName[ATTR_SIZE], AttrCatEntry *attrCatBuf)
{
    ...
    for (AttrCacheEntry *entry = attrCache[relId]; entry != nullptr; entry = entry->next)
    {
        // match by attribute name instead of offset
        // strcmp returns 0 when both strings are equal
        if (strcmp(entry->attrCatEntry.attrName, attrName) == 0)
        {
            // overwrite the cached AttrCatEntry fields with values from attrCatBuf
            // strcpy used for char array fields, direct assignment for primitive fields
            strcpy(entry->attrCatEntry.relName, attrCatBuf->relName);
            strcpy(entry->attrCatEntry.attrName, attrCatBuf->attrName);
    ...
    // no attribute with the given name exists in this relation's cache
    return E_ATTRNOTEXIST;
}

void AttrCacheTable::attrCatEntryToRecord(AttrCatEntry *attrCatEntry, Attribute record[ATTRCAT_NO_ATTRS])
{
    // convert the struct fields to Attribute union format for disk storage

    // relName and attrName are string fields — copy into sVal of the Attribute union
    strcpy(record[ATTRCAT_REL_NAME_INDEX].sVal, attrCatEntry->relName);
    strcpy(record[ATTRCAT_ATTR_NAME_INDEX].sVal, attrCatEntry->attrName);

    // numeric fields — assign into nVal of the Attribute union
    // attrType: 0 for NUMBER, 1 for STRING (as defined in NITCbase constants)
    record[ATTRCAT_ATTR_TYPE_INDEX].nVal    = attrCatEntry->attrType;

    // primaryFlag: 1 if this attribute is part of the primary key, 0 otherwise
    record[ATTRCAT_PRIMARY_FLAG_INDEX].nVal = attrCatEntry->primaryFlag;

    // rootBlock: block number of the root of the B+ tree index on this attribute
    // -1 if no index exists
    record[ATTRCAT_ROOT_BLOCK_INDEX].nVal   = attrCatEntry->rootBlock;

    // offset: position of this attribute in the relation's record (0-indexed)
